...
void putText(sf::RenderWindow &window, const sf::Font &font,
             const std::string &s, float cx, float cy, unsigned int size,
...
void MenuScreen::drawButton(const std::string &label,
                            const sf::FloatRect &bounds, bool disabled,
                            bool hovered) {
  ...
  const sf::Color textColor =
      disabled ? hexColor(0x777788) : hexColor(0xffffff);
  putText(window, font, label, bounds.position.x + bounds.size.x / 2.f,
          bounds.position.y + bounds.size.y / 2.f, 22u, textColor, true);
}

void MenuScreen::render(int scorRunda, int scorTotal, int vieti,
                        bool showScores, bool esteRecord) {
  ...
  if (showScores) {
    ...
    if (esteRecord) {
      putText(window, font, " NOU RECORD! ", WINDOW_W / 2.f, 278.f, 26u,
              hexColor(0xffd54f), true);
    }
  } else {
  ...
  putText(window, font,
          "C  ->  Clasament    |    Vieti ramase: " + std::to_string(vieti),
...
void MenuScreen::renderClasament(const Clasament<int> &scoruri,
  ...
  putText(window, font,
          "Top scoruri (" + std::to_string(scoruri.marime()) + "/5)", 230.f,
          150.f, 22u, hexColor(0xffd54f), true);
  ...
  for (std::size_t i = 0; i < sIntrari.size(); ++i) {
    const std::string linie = std::to_string(i + 1) + ". " + sIntrari[i].nume +
